...
def manhole_process(image,center,height,width,src_color=(0,0,0)):
    ...
    # OpenCV ellipse parametrelerini düzelt
    try:
        # Center'ı integer tuple'a çevir ve (x,y) formatında olduğundan emin ol
        center = (int(center[0]), int(center[1]))
        # Axes yarı-eksen uzunlukları olmalı
        axes = (int(width//2), int(height//2))
        cv2.ellipse(overlay, center, axes, 0, 0, 360, src_color, -1)
    except Exception:
        # Fallback: Circle kullan
        center = (int(center[0]), int(center[1]))
        radius = min(int(width//2), int(height//2))
...
def add_manhole(image,center=-1,color=(120,120,120),height=1,width=1, type='closed'): ##function to flip the image on horizontal axis
    ...
    if(is_list(image)):
        ...
        for img in image_list:
            ...
            if center==-1:
                center_t= (img.shape[1]//2, img.shape[0]-100)  # (x, y) formatında
        ...
        if center==-1:
            center_t= (image.shape[1]//2, image.shape[0]-100)  # (x, y) formatında
